...
public class Gossiper implements IFailureDetectionEventListener, IEndPointStateChangePublisher
{
    ...
    //public final static int intervalInMillis_ = 500;
    ...
    */
	...
	//PALLAVI: sleep for sometime to let all servers come up
	//Util.sleep(1000);

	//PALLAVI: Send gossip to all seeds
	...
	Util.debug("Sending a GossipDigestSynMessage to " + to + " ...");
	...
	******************/
    ...
    //PALLAVI: gossip only once
    public void doGossipOnce(){
         try {
		synchronized (Gossiper.instance) {
		    /* Update the local heartbeat counter. */
                    endPointStateMap_.get(localEndPoint_).getHeartBeatState().updateHeartBeat();
    ...
    */
	...
	//PALLAVI: check when this server considers all seeds to be alive
	//if(liveEndpoints_.size() == seeds_.size()){
	//	FMClient.gossipDone();
	//}
    }
    ...
    */
	...
	//PALLAVI: gossip once instead of doing it continuously
