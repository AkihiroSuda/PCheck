...
If you haven't read the hdfs-fi/README.txt file, then 
stop reading this. Read that README first. Then continue here.


[[ 2. How to Setup the Configuration ]]

To build the hdfs-wd, you need to setup some paths 
...
- Modify the property locations of the files (don't modify the
  ...
  The "hadoop.root.dir" is the path to your hdfs-fi directory.  If you put
  hdfs-fi and hdfs-wd in the same directory, then the location is
  "../hdfs-fi".
...
Simply, run these steps:
 ...
 % ./scripts/run-brute.py 1
   ...
   (THIS WILL TAKE A WHILE, because it's excercising all possible
   failure points in HDFS. Meanwhile you could see /tmp/fi/expResult/
   folder to see how many experiments have been run so far.
